...
contract MEME is ERC20Burnable, Ownable2Step, ReentrancyGuard {
    ...
    event Price(uint256 time, uint256 price, uint256 volumeInLarry);
    ...
    constructor(address _larry, string memory _name, string memory _symbol) ERC20(_name, _symbol) Ownable(msg.sender) {
        backingToken = IERC20(_larry);
    }
    ...
    function buy(address receiver, uint256 amount) external nonReentrant {
        ...
        uint256 tokens = LARRYtoTOKENS(amount);
        backingToken.safeTransferFrom(msg.sender, address(this), amount);
        ...
        // Burn LARRY tokens using the burnable function
        ERC20Burnable(address(backingToken)).burn(burnAmount);
    ...
    function sell(uint256 tokens) external nonReentrant {
        ...
        // AUDIT: to user round down
        uint256 larry = TOKENStoLARRY(tokens);
        ...
        backingToken.safeTransfer(
            msg.sender,
            (larry * sell_fee) / FEE_BASE_1000
        );

        // Team fee: 1% total -> 0.8% to FEE_ADDRESS, 0.2% burned
        uint256 totalFeeAmount = larry / FEES_SELL;
    ...
    function leverageFee(
        ...
        uint256 mintFee = (larry * buy_fee_leverage) / FEE_BASE_1000;

        uint256 interest = getInterestFee(larry, numberOfDays);

        return (mintFee + interest);
    }

    function leverage(uint256 larry, uint256 numberOfDays) public nonReentrant {
        require(start, "Trading must be initialized");
        require(
            numberOfDays < 366,
        ...
        uint256 larryFee = leverageFee(larry, numberOfDays);

        uint256 userLarry = larry - larryFee;

        uint256 feeAddressAmount = (larryFee * 3) / 10; // 30% to FEE_ADDRESS  
        uint256 burnFeeAmount = (larryFee * 1) / 10; // 10% burned
        uint256 userBorrow = (userLarry * 99) / 100;
        uint256 overCollateralizationAmount = (userLarry) / 100;
        uint256 totalFee = (larryFee + overCollateralizationAmount);

        // AUDIT: to user round down
        uint256 userTokens = LARRYtoTOKENS(userLarry);
        ...
        backingToken.safeTransfer(FEE_ADDRESS, feeAddressAmount);
        // Burn LARRY tokens using the burnable function
        ERC20Burnable(address(backingToken)).burn(burnFeeAmount);
    ...
    function borrow(uint256 larry, uint256 numberOfDays) public nonReentrant {
        require(
            numberOfDays < 366,
            "Max borrow/extension must be 365 days or less"
        );
        ...
        uint256 larryFee = getInterestFee(larry, numberOfDays);

        uint256 feeAddressFee = (larryFee * 3) / 10; // 30% to FEE_ADDRESS
        uint256 burnFeeAmount = (larryFee * 1) / 10; // 10% burned

        //AUDIT: tokens required from user round up?
        uint256 userTokens = LARRYtoTOKENSCeil(larry);

        uint256 newUserBorrow = (larry * 99) / 100;
        ...
        backingToken.safeTransfer(msg.sender, newUserBorrow - larryFee);
        ...
        emit UserLoanDataUpdate(msg.sender, Loans[msg.sender]);
        safetyCheck(larryFee);
    }
    function borrowMore(uint256 larry) public nonReentrant {
        require(!isLoanExpired(msg.sender), "Loan expired use borrow");
        require(larry != 0, "Must borrow more than 0");
        liquidate();
        ...
        uint256 larryFee = getInterestFee(larry, newBorrowLength);

        //AUDIT: tokens required from user round up?
        uint256 userTokens = LARRYtoTOKENSCeil(larry);
        uint256 userBorrowedInTokens = LARRYtoTOKENS(userBorrowed);
    ...
    function removeCollateral(uint256 amount) public nonReentrant {
        ...
        require(
            Loans[msg.sender].borrowed <=
                (TOKENStoLARRY(collateral - amount) * 99) / 100,
    ...
    function flashClosePosition() public nonReentrant {
        ...
        uint256 collateralInLarry = TOKENStoLARRY(collateral);
        _burn(address(this), collateral);

        uint256 collateralInLarryAfterFee = (collateralInLarry * 99) / 100;

        uint256 fee = collateralInLarry / 100;
        require(
            collateralInLarryAfterFee >= borrowed,
            "You do not have enough collateral to close position"
        );

        uint256 toUser = collateralInLarryAfterFee - borrowed;
    ...
    function safetyCheck(uint256 larry) private {
        ...
        lastPrice = newPrice;
        emit Price(block.timestamp, newPrice, larry);
    }

    function TOKENStoLARRY(uint256 value) public view returns (uint256) {
        return Math.mulDiv(value, getBacking(), totalSupply());
    }

    function LARRYtoTOKENS(uint256 value) public view returns (uint256) {
        return Math.mulDiv(value, totalSupply(), getBacking());
    }

    function LARRYtoTOKENSCeil(uint256 value) public view returns (uint256) {
